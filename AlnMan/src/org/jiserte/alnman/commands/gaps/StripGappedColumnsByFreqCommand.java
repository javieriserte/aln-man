 ...
 *
 */
public class StripGappedColumnsByFreqCommand extends FastaCommand<SingleArgumentOption<Double>> {
	
	SingleArgumentOption<File> reference;
	
	public StripGappedColumnsByFreqCommand(InputStream inputstream, PrintStream output, SingleArgumentOption<Double> option, SingleArgumentOption<File> reference) {
	...
	protected List<String> performAction() {
		...
		double fr = this.getOption().getValue();
		...
		if (this.getReference().isPresent()) {
			...
			outreference = this.getReference().getValue();
	...
	public SingleArgumentOption<File> getReference() {
		return reference;
	}

	public void setReference(SingleArgumentOption<File> reference2) {
